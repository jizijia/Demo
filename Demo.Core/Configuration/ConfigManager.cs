...
namespace Demo.Core.Configuration
{
    ...
    public class ConfigManager
    {
        ...
        protected static Dictionary<Type, XmlSerializer> configSerializers = new Dictionary<Type, XmlSerializer>();

        protected static XmlSerializer GetSerializer(Type type)
        {
            if (configSerializers.ContainsKey(type))
                return configSerializers[type];
        ...
        protected static MemoryCacheManager configCache = new MemoryCacheManager();
        ...
        public static T GetConfig<T>()
        {
            ...
            else
            {
                ...
                if (config == null)
                {
                    ...
                    if (config != null)
                    {
                        configCache.Set(configType.FullName, config, path);
                    }
                }
            }
            return config;
